...
Kiwi 모델(~400MB)은 서버 프로세스에 상주하지 않고 별도 자식 프로세스(`kiwi_worker`)에서만 적재됩니다(2.5.1). 다만 이 워커는 **첫 호출 때가 아니라 서버가 뜨는 즉시** 함께 기동합니다(`db.open_db()`가 `tokenizer.ensure_ready()`를 무조건 호출) — "요청이 없으면 안 뜬다"가 아니라 "요청이 없으면 (`AUSTIN_POWER_KIWI_IDLE`초 후) 스스로 내려간다" 쪽이 맞는 설명입니다. 그래서 메모리 수치는 하나가 아니라 둘로 나뉩니다:

- **서버 프로세스 자체**(워커를 제외한 서버 하나의 RSS — 워커가 떠 있든 유휴 종료로 내려갔든 무관) — 약 **69.5MB**. 기동 직후에는 워커도 같이 떠 있으므로 시스템 전체로는 이보다 크고, 워커가 유휴 종료된 뒤에야 실제 합계가 이 수치에 수렴합니다.
