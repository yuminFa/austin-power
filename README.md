...
3. 워커가 결과를 한 번 더 검사합니다.
   ...
   - 갱신이어도 `kind`는 기존 값을 유지합니다.
   - 토큰·비밀번호처럼 보이는 값(GitHub·AWS·Slack 토큰, PEM 개인키 등)이 든 기억은 본문을 LLM에 보내지 않고(제목에 들어 있으면 항목째 제외) 갱신 대상에서도 뺍니다.
4. 같은 프로젝트의 추출은 프로젝트별 파일 락으로 한 번에 하나씩 돌아, 두 워커가 서로의 병합을 덮어쓰지 않습니다.

주의: 1번 때문에 같은 프로젝트의 기존 기억 본문도(세션 텍스트와 마찬가지로) 추출 CLI 제공자에게 전송됩니다. 비밀 값 탐지는 대표 패턴만 잡는 완화책이므로, 비밀은 기억에 저장하지 마세요.
