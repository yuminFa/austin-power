...
- [x] U8'. 2.5.1 워커 분리 후 재측정(2026-09-23, macOS arm64, `scripts/measure_rss.py`) — 목표치는 2.5.1 "효과 목표", 4개 항목 전부 합격:
  - 유휴 서버 RSS(서버 프로세스 단독, `save`/`search` 호출 전 — 단 워커 자체는 `serve()`가 `db.open_db()`에서 `tokenizer.ensure_ready()`를 무조건 호출하므로 기동 시점에 이미 함께 뜬다) **69.5MB** ≤ 120MB → **PASS**
  - 사용 중 서버+워커 합계 RSS(1,000회 `save`/`search` 호출 동안 관측한 최대치) **580.0MB** ≤ 650MB → **PASS**
  - `AUSTIN_POWER_KIWI_IDLE=2`로 기동 → 첫 호출로 워커 확인 → 4초 대기(워커 종료 확인) → 다음 호출 지연 **0.87s** ≤ 3s → **PASS**
  - 노트 1,000개 저장 후 `meta.tokenizer_sig` 강제 불일치 → 서버 재기동(기동~`/health` 응답, 워커 재기동·모델 재적재·FTS5 재색인 전부 포함) **1.22s** ≤ 15s → **PASS**
  - 참고로 기존 U8 방식(기동 직후 대비 1,000회 호출 후 증가율)도 재측정: 70.1MB → 71.4MB, growth_pct **1.8%**(worker로 Kiwi가 분리되면서 서버 프로세스 자체는 더 이상 첫 호출에 380MB대 모델을 물지 않으므로, 워커 분리 전 34.2%였던 이 수치도 20% 기준을 통과한다 — 다만 이 수치는 서버 프로세스만 보므로 §2.5.1 "사용 중 합계" 기준(650MB)이 실질적 예산선)
...
- [x] 메모리: U8'(2.5.1 워커 분리 후) 기준 — 실측 완료, 4개 항목 전부 합격(수치는 U8' 항목 참고)
